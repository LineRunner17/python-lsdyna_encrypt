...
def ask_overwrite(question):
    while True:
        print(f"{CliColors.PR_CRITICAL}{question} Overwrite? (y/n){CliColors.reset_all}")
        overwrite = input().lower().strip()
        if overwrite == 'y':
            break
        elif overwrite == 'n':
            print(f"{CliColors.PR_ERROR}User canceled.")
            sys.exit()
        else:
            print(f"{CliColors.PR_ERROR}invalid input. Try again.")
...
$
$ LS-Dyna Material-File encryption script (generated by version {self.__version__})
$
$ user: {user_id}
$ date: {today_date}
$
...
def start_args():
    ...
    my_parser.version = LS_Dyna_Mat_Encryptor.__version__
    ...
    return args.inputfile, args.outfile, args.key_length, args.expiry_date
